...
for (i in list(7,8,12,13,14,15,17,19,20,21,22,23,24,25,27)) {
  ...
  plot(cars[[columns[i]]], maxPrice, xlab = columns[i], ylab = "MaximumPrice")
  plot(log(cars[[columns[i]]]), maxPrice, xlab = paste("Log(",columns[i],")"), ylab = "MaximumPrice")
  plot(cars[[columns[i]]], log(maxPrice), xlab = columns[i], ylab = "Log(MaximumPrice)")
  plot(log(cars[[columns[i]]]), log(maxPrice), xlab = paste("Log(",columns[i],")"), ylab = "Log(MaximumPrice)")
...
for (i in list(9,10,11,16,18,26)) {
  ...
  boxplot(maxPrice~cars[[columns[i]]], xlab = columns[i], ylab = "MaximumPrice")
  boxplot(log(maxPrice)~cars[[columns[i]]], xlab = columns[i], ylab = "Log(MaximumPrice)")
}
...
for (i in list(7,8,12,13,14,15,17,19,20,21,22,23,24,25,27)) { 
  ...
  print(cor.test(cars[[columns[i]]], maxPrice, method="spearman"))
...
for (i in list(9,10,11,16,18,26)) {
  ...
  print(summary(aov(maxPrice~factor(cars[[columns[i]]]))))
  print(summary(aov(log(maxPrice)~factor(cars[[columns[i]]]))))
...
for (i in list(9,10,11,16,18)) {
  ...
  mod.var <- aov(maxPrice~factor(cars[[columns[i]]]))
  mod.logvar <-aov(log(maxPrice)~factor(cars[[columns[i]]]))
